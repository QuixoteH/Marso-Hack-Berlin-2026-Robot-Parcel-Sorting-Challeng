...
def main(cfg):
    ...
    # Diffusion-policy action sampling consumes Torch RNG. Environment seeds alone
    # do not make a fixed-episode comparison reproducible.
    torch.manual_seed(0)
    ...
    if bool(cfg.get("record_video", True)):
        ...
        print(
            f"[eval] saved rollout video (render + sensor views) -> {vid_dir}",
            flush=True,
        )
