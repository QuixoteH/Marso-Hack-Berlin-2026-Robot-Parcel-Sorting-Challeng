...
class Args:
    ...
    """Train from the demonstration H5 only; used on hosts without a Vulkan renderer."""
...
if __name__ == "__main__":
    ...
    if args.skip_env_eval:
        example = dataset.trajectories['observations'][0]
        envs = types.SimpleNamespace(
            single_observation_space={
                "state": types.SimpleNamespace(shape=(example['state'].shape[-1],)),
                "rgb": types.SimpleNamespace(shape=(1, 3, 224, 224)),
            },
            single_action_space=types.SimpleNamespace(shape=(dataset.trajectories['actions'][0].shape[-1],)),
        )
    else:
        other_kwargs = None
    ...
    for cur_iter, data_batch in enumerate(train_dataloader):
        ...
        # Evaluation
        if not args.skip_env_eval and cur_iter % args.eval_freq == 0:
            last_tick = time.time()
    ...
    torch.save({
        ...
        'agent': agent.state_dict(),
        'ema_agent': ema_agent.state_dict(),
    }, f'runs/{run_name}/checkpoints/final.pt')
