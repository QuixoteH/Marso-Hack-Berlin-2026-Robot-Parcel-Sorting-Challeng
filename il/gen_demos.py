...
def record_raw_demos(
    ...
    env = gym.make(
        ...
        sim_backend="gpu",
        # Raw state recording does not need a renderer.  Keeping rgb_array here makes
        # --no-replay --no-media still initialize Vulkan on headless servers.
        render_mode=None,
        render_backend="none",
    ...
    seeds = list(episode_seeds) if episode_seeds is not None else [
        base_seed + i for i in range(num_episodes)
    ]
    print(f"recording {len(seeds)} episodes  difficulty={difficulty}", flush=True)
    n_success = 0
    ...
    print(f"\nrecorded {len(seeds)} episodes  ({n_success} full-success)  -> {h5}", flush=True)
    return h5
...
def main():
    ...
    ap.add_argument("--seeds", type=int, nargs="+", default=None,
                    help="explicit episode seeds; overrides --num-episodes and --base-seed")
